                  ...
                  <button 
                    onClick={() => {
                      if (typeof window !== 'undefined') {
                        window.open('https://farcaster.xyz/~/mini-apps/launch?domain=wagmi-blaster.vercel.app', '_blank')
                      }
                    }}
...
function WalletConnectSection() {
  ...
  const isOnCorrectChain = chain?.id === ARBITRUM_CHAIN_ID
  ...
  async function authenticatedFetch(input: RequestInfo | URL, init?: RequestInit) {
    ...
    // compute fused key using ethers via dynamic import to avoid SSR issues
    const { keccak256, toUtf8Bytes } = await import('ethers')
    const fused = keccak256(toUtf8Bytes(secret + random)).slice(2)
    const headers = new Headers(init?.headers || {})
    headers.set('x-random-string', random)
    headers.set('x-fused-key', fused)
      ...
      {!isConnected ? (
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top duration-500">
          ...
          <button
            ...
            disabled={!name.trim() || isSubmitting || !isOnCorrectChain}
          ...
          >
            {!isSubmitting && name.trim() && isOnCorrectChain && (
          ...
          <button
            onClick={() => disconnect()}
            className="w-full bg-gradient-to-r from-red-500/80 to-red-600/80 hover:from-red-600/90 hover:to-red-700/90 backdrop-blur-sm text-white font-semibold py-4 px-6 rounded-2xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 border border-red-400/30"
            disabled={isSubmitting}
          >
            Disconnect Wallet
          </button>
