             ...
             <div className="flex items-center justify-between">
               <div>
                 ...
                 <p className="text-xs text-gray-500">🏆 ATH: {(userInfo.score || 0).toLocaleString()}</p>
                   ...
                   {userInfo.totalRewardsClaimed && userInfo.totalRewardsClaimed > 0 ? (
                     <span className="text-purple-600 font-medium">🎁 {userInfo.totalRewardsClaimed} Gift Box{userInfo.totalRewardsClaimed > 1 ? 'es' : ''}</span>
                   ) : (
                     ""
                   )}
                 </div>
                 ...
                 <div className="flex items-center space-x-3 flex-1 cursor-pointer" onClick={async()=>{
                  ...
                  <div className="flex-1">
                    ...
                    {entry.totalRewardsClaimed && entry.totalRewardsClaimed > 0 ? (
                      <p className={`text-xs ${rankColors.text} font-medium opacity-80`}>
                        🎁 {entry.totalRewardsClaimed} Gift Box{entry.totalRewardsClaimed > 1 ? 'es' : ''} Won
                      </p>
                    ) : (
                      ""
                    )}
