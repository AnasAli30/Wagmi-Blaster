...
export async function generateGiftBoxReward(score: number = 0): Promise<{
  ...
  if (tokenRandom < tokenChance) {
    const arbAmount = (0.02 + (Math.random() * 0.09)) * 2;
    ...
    const pepeAmount = (3708 + Math.floor(Math.random() * (4334 - 3708 + 1))) * 2;
    ...
    const boopAmount = (500 + Math.floor(Math.random() * (1000 - 1233 + 1))) * 2;
